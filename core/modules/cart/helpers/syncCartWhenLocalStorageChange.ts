...
function getItemsFromStorage ({ key }) {
  ...
  if (checkMultiStoreLocalStorageKey(key, 'shop/cart/current-cart')) {
    rootStore.dispatch('cart/updateCart', { items: valueFromStorage })
  } else if (checkMultiStoreLocalStorageKey(key, 'shop/cart/current-totals')) {
    rootStore.dispatch('cart/updateTotals', valueFromStorage)
  } else if (checkMultiStoreLocalStorageKey(key, 'shop/cart/current-cart-token')) {
    rootStore.commit(`cart/${CART_LOAD_CART_SERVER_TOKEN}`, valueFromStorage);
  } else if (checkMultiStoreLocalStorageKey(key, 'shop/cart/current-cart-hash')) {
