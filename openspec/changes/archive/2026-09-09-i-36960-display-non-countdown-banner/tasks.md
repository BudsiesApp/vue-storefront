...
- [x] 2.2 Preserve the existing location, single selected banner, title and rich-text directives, colors, blacklist rules, responsive controls, and version-based close behavior while omitting the rendered banner container when its processed content is not meaningful.
...
- [x] 4.2 Implement one Vue-owned shortcut state for idle, applying, saved, applied, already-applied, conflict, invalid, and failed outcomes, including a single in-flight guard and stale-banner result protection.
